...
public class NugetRunStep extends AbstractStepImpl {
    ...
    public static class Execution extends ArtifactorySynchronousNonBlockingStepExecution<Void> {
        ...
        private void setResolver(String buildNumber) throws IOException, InterruptedException {
            ...
            BuildDataFile buildDataFile = DeclarativePipelineUtils.readBuildDataFile(ws, buildNumber, NugetResolverStep.STEP_NAME, step.resolverId);
            if (buildDataFile == null) {
                throw new IOException("Resolver " + step.resolverId + " doesn't exist!");
            }
            CommonResolver resolver = SerializationUtils.createMapper().treeToValue(buildDataFile.get(NugetResolverStep.STEP_NAME), CommonResolver.class);
